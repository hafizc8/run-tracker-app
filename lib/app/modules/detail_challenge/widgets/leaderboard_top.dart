...
class Top3LeaderBoardIndividualList extends StatelessWidget {
  final List<LeaderboardUser> topWalkers;

  const Top3LeaderBoardIndividualList({super.key, required this.topWalkers});
        ...
        children: [
          ...
          if (firstPlace != null)
            _PodiumWalkerProfile(
              walker: firstPlace,
              isFirstPlace: thirdPlace == null
                  ? (secondPlace == null ? true : false)
                  : true,
            )
          else
    ...
    return Expanded(
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                    shape: BoxShape.circle, color: darkColorScheme.primary),
              ),

              // Gambar profil pengguna
              Padding(
                padding:
                    const EdgeInsets.all(4.0), // Jarak untuk ketebalan border
                child: ClipOval(
                  child: CachedNetworkImage(
                    ...
                    placeholder: (context, url) =>
                        ShimmerLoadingCircle(size: avatarSize - 8),
                    errorWidget: (context, url, error) => CircleAvatar(
                      radius: 32.r,
                      backgroundColor:
                          Theme.of(context).colorScheme.onBackground,
                      child: Text(
                        (walker.user?.name ?? '').toInitials(),
                      ),
                    ),
                  ),
                ),
          ...
          SizedBox(
              height: rankBadgeSize / 2 +
                  4.h), // Spasi agar teks tidak tertutup lencana
          ...
          // Jumlah langkah
          Text(
            NumberFormat('#,###', "id_ID").format(walker.point ?? 0),
            style: Theme.of(context).textTheme.titleSmall?.copyWith(
