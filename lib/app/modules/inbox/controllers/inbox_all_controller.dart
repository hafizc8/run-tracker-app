...
class InboxAllController extends GetxController {
  ...
  void goToRoomChatEvent(ChatInboxModel chat) {
    Get.toNamed(AppRoutes.eventChat, arguments: {
      'id': chat.relateableEvent?.id ?? '',
      'title': chat.relateableEvent?.title ?? '',
    });
  }

  void goToRoomChatClub(ChatInboxModel chat) {
    Get.toNamed(AppRoutes.clubChat, arguments: {
      'id': chat.relateableClub?.id ?? '',
      'title': chat.relateableClub?.name ?? '',
      'imgUrl': chat.relateableClub?.imageUrl ?? '',
    });
  }
}
